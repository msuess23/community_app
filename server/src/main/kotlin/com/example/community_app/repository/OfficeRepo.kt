...
object DefaultOfficeRepository : OfficeRepository {
  ...
  override suspend fun create(dto: OfficeCreateData): OfficeRecord =
    newSuspendedTransaction(Dispatchers.IO) {
      ...
      (Offices innerJoin Locations)
        .selectAll()
        .apply { andWhere { Offices.id eq office.id } }
        .first()
        .toOfficeRecord()
    }
  ...
  override suspend fun update(id: Int, patch: OfficeUpdateData): OfficeRecord? =
    newSuspendedTransaction(Dispatchers.IO) {
      ...
      patch.location?.let {
        office.location.longitude = it.longitude
        office.location.latitude = it.latitude
        office.location.altitude = it.altitude
        office.location.accuracy = it.accuracy
      }
  ...
  override suspend fun delete(id: Int) {
    newSuspendedTransaction(Dispatchers.IO) {
      Offices.deleteWhere { Offices.id eq id }
    }
  }
